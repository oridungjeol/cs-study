...
**main.o 내부**
- `hello()` 호출하는 부분은 `hello`라는 심볼 이름만 있고 주소는 아직 모름.
- 심볼 테이블에 `hello`가 외부 함수로 참조됨.

**hello.o 내부**
- `hello` 함수가 정의됨.
- 심볼 테이블에 `hello`가 정의된 심볼로 등록됨.
...
- 덕분에 함수 호출이나 변수 접근 시 사용하는 주소가 올바르게 바뀌어,
- 프로그램이 정확히 동작할 수 있게 됨.
